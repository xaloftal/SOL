...
module.exports = {
    ...
    GetMedicosByEspecialidade: (req, res) => {
        client.query('SELECT m.*,e.* FROM medico m INNER JOIN especialidade e ON m.id_especialidade = e.id_especialidade where m.id_especialidade = $1;', [req.query.id], (error, results) => {
            if (error) {
                throw error
            }
            res.send(results.rows)
